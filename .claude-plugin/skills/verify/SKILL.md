...
`pass` 只表示当前 phase 的产物和证据已验证通过，不表示代码已经 merge。
如果存在外部 PR / review 流程，是否已 merge 只能作为补充上下文记录，不能替代当前 skill 的验证结论。
...
- 当前 phase `Type = tech` 且 `partial|fail`：优先使用 `verification.md` 中的 `recommended next step`；若未明确写出，则回到当前 phase 的 `Owner Skill`
- 当前 phase `Type = requirement` 且 `partial`：若问题属于计划覆盖不足，进入 `pace:plan`；若问题属于实现未完成，进入 `pace:execute`
- 当前 phase `Type = requirement` 且 `fail`：默认进入 `pace:execute`，修复后再回 `pace:verify`
