...
8. 输出摘要并推荐下一步
9. 若当前是 `multica + github`：
   - 先执行 `node "$HOME/.codex/skills/pace/bin/pace-issue-doc.js" ensure-root --issue <main-issue>`
   - 若已确定当前 requirement 所属 phase，则默认使用 `doc-key = phase-<NN>`
   - 将 intake 结论同步到 phase 文档 issue，至少更新 `--section requirement-summary`
   - 若 intake 仅修改 roadmap 归属而未确定 phase，则至少更新主 issue comment、文档 root issue 索引和初始化参数文档 issue，不允许只留本地 `.pace/*`
