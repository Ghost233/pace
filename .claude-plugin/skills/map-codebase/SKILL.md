...
如果配置文件存在，提取 `agents.max_concurrent`、`agents.model_profile`、`agents.model_overrides` 并应用于后续流程。`agents.max_concurrent` 直接控制子代理并行数上限。
...
- `.pace/codebase/*` 属于当前工作区缓存与辅助真相源，供下游 skills 直接复用
