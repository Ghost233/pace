...
如果配置文件存在，提取 `tracker`、`agents.max_concurrent`、`agents.model_profile`、`agents.model_overrides` 并应用于后续流程。`agents.max_concurrent` 直接控制子代理并行数上限。
...
- `multica + github` 下，本地 `.pace/codebase/*` 只在工作区已从 GitHub 主 issue、主 issue 的受控索引 comment、文档 root issue、初始化参数文档 issue、相关文档 issue 恢复后才可信；若检测到缺失恢复、状态冲突或副本不完整，必须先停止并要求恢复/同步
- `multica + github` 下，`.pace/codebase/*` 仍是工作区缓存；如果当前轮需要把这批代码地图作为跨轮次证据保留，必须由角色或文档层协议把摘要/正文同步到 GitHub 文档链，不能把本地文件当成第二真相源
...
4. 若当前是 `multica + github`，且本轮需要把代码地图作为跨轮次证据保留：
   - 先执行 `node "$HOME/.codex/skills/pace/bin/pace-issue-doc.js" ensure-root --issue <main-issue>`
   - 再执行 `upsert-doc` 同步 `codebase-stack`、`codebase-architecture`，以及扩展模式下的其他代码地图文档
   - 如有增量更新说明，配套写审计 comment
   - 只有当文档 root issue 与主 issue 受控索引 comment 已回填最新索引后，才算 GitHub 文档层同步完成
...
3. 为检测到变更的领域启动子代理并行更新：
   ...
   - 在更新文件头部标注更新日期
4. 完成后由主代理确认更新一致性
5. 若当前是 `multica + github`，且本轮需要把更新后的代码地图作为跨轮次证据保留：
   - 先执行 `node "$HOME/.codex/skills/pace/bin/pace-issue-doc.js" ensure-root --issue <main-issue>`
   - 再执行 `upsert-doc` 同步受影响的代码地图文档
   - 如有增量更新说明，配套写审计 comment
   - 只有当文档 root issue 与主 issue 受控索引 comment 已回填最新索引后，才算 GitHub 文档层同步完成
