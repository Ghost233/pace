...
- 主 issue 的受控索引 comment 负责回填文档 root issue 与文档索引
- 文档 root issue 负责当前文档索引
- 初始化参数文档 issue 负责当前角色复用的初始化参数
...
- 更新后的 `.pace/state.md`
- 若当前是 `multica + github`，还必须同步对应 GitHub 文档层：
  - `ensure-root`
  - 文档 root issue 索引更新
  - 主 issue 受控索引 comment 回填
  - 受影响 phase 的 phase 文档 issue section 更新
...
当 `tracker.type = github` 且 `executor = multica` 时：

- phase 结构变更不能只改本地 `.pace/roadmap.md` / `.pace/state.md`
- 必须通过 `node "$HOME/.codex/skills/pace/bin/pace-issue-doc.js" ensure-root --issue <main-issue>` 保证文档链存在
- 若当前 phase 已确定，默认把 roadmap 结构变更摘要同步到对应 `phase-<NN>` 文档 issue 的 `requirement-summary` 或 `plan` section
- 若 phase 尚未确定，也必须至少回填主 issue 的受控索引 comment 与文档 root issue 索引
