...
function main() {
  ...
  const bundledPaceDir = path.resolve(path.dirname(fs.realpathSync(__filename)), '..', '.pace');
  ...
  const baseFile = fs.existsSync(localBaseFile) ? localBaseFile : bundledBaseFile;
  const envFile = fs.existsSync(localEnvFile) ? localEnvFile : bundledEnvFile;
